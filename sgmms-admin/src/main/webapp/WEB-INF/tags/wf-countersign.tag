...
<%@attribute name="agreeActivityKey" required="true" type="java.lang.String" description="当同意时流程走的节点的key，只有showAgree为true时才有效" %>
<%@attribute name="rejectActivityKey" required="true" type="java.lang.String" description="当不同意时流程走的节点的key，只有showAgree为true时才有效" %>
...
<%@attribute name="canSelectHander" required="false" type="java.lang.Boolean" description="是否可以选择任务处理人" %>
...
<c:if test="${empty canSelectHander}">
	<c:set var="canSelectHander" value="false"/>
</c:if>
...
<script type="text/javascript">
	...
	function createActivityDIV(activityId,activityName,multiSelect,checked,sendEmail){
		var activityDIV =  "<div class='col-sm-2 radio'>"+
							  "<label>"+
							      "<input type='radio' value='" + activityId + "' multiSelect='" + multiSelect + "' name='nextActivityId' sendEmail='" + sendEmail + "' " + checked + " > " + activityName +
							  "</label>"+
							"</div>";
	...
	*/
	...
	function agreeChangeState(state) {
	    ...
	    if("Y" == state){
	    	activityChangeState("${pageScope.agreeActivityKey}");
			...
			activityChangeState("${pageScope.rejectActivityKey}");
	...
	*/
	...
	function activityChangeState(activityId){
		...
		//隐藏不相关的节点
		$.each(activityRads,function(j,activityRad){
			if($(this).val() == activityId){
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <div class="form-group">
                ...
                <div class="col-sm-10" id="activityDIV"></div>
            </div>
